...
app (projectName, function(task)
{
    ...
    task.setBuildIntermediatesPath(FILE.join("Build", "DisapearingViews.build", configuration));
    task.setBuildPath(FILE.join("Build", configuration));

    task.setProductName("DisapearingViews");
    task.setIdentifier("com.krodelin.DisapearingViews");
    task.setVersion("1.0");
    task.setAuthor("Krodelin Software Solutions");
    task.setEmail("info@krodelin.com");
    task.setSummary("DisapearingViews");
...
task ("desktop", ["release"], function()
{
    ...
    require("cappuccino/nativehost").buildNativeHost(FILE.join("Build", "Release", projectName), FILE.join("Build", "Desktop", projectName, "DisapearingViews.app"));
    printResults("Desktop")
});

task ("run-desktop", ["desktop"], function()
{
    OS.system([FILE.join("Build", "Desktop", projectName, "DisapearingViews.app", "Contents", "MacOS", "NativeHost"), "-i"]);
});
